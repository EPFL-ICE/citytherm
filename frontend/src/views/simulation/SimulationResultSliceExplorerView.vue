...
import { type TimeSeriesPoint, useScenariosStore, type ScenarioDescription } from '@/stores/scenarios'
...
import { getSimulationPlaneAvailableTimeSlots, getSimulationPlanePresetsForParameters, type SimulationPlanePreset, type SimulationPlanePresetsMap } from '@/lib/simulation/simulationResultPlanesUtils'
        ...
        <template #footer>
          <div>
            <v-select
              ...
              :item-props="(item) => ({ title: `x: ${item.c[0]} ; y: ${item.c[1]} ; z: ${item.c[2]}`, subtitle: availablePlanes[item.p as SimulationPlanePreset].name, value: makePointSlugArray(item.c) })"
              ...
              class="mb-2"
            />
            <v-btn color="primary" :disabled="!selectedTimeSeriesPointSlug" :to="timeSeriesExplorerUrl!"> Explore point time series </v-btn>
          </div>
        </template>
      </tool-set>
    </template>
