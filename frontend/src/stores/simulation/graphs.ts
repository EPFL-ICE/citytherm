import { graphKindsDescriptions, type GraphKind } from "@/components/simulation/pickers/graphKindPickerUtils";
import { getSimulationPresetsForScenarioSlug, type SimulationPlanePreset } from "@/lib/simulation/simulationResultPlanesUtils";
...
export const useGraphsStore = defineStore("graphs", () => {
  ...
  function createGraph(input: CreateGraphDesignInput): AsyncResult<GraphDesign> {
    return AsyncResult.run(function* () {
      ...
      const scenarioNames = input.scenarios.map(slug => {
      ...
      if (input.plane) {
        ...
        const available = yield* AsyncResult.fromValuePromise(scenariosStore.getAvailableTimeSeriesPointsForScenario(input.scenarios[0]))
      ...
      const design: GraphDesign = {
        id,
        name: `${graphKindsDescriptions[input.graphKindSlug].shortName} | ${scenarioNames} | ${location}`,
        description: '',
        time: 'time_12',
        variables: [],
        ...input
