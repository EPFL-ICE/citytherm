...
async function fetchSimulationResultTimeSeriesForScenarioVariableAndPoint(scenarioSlug: string, variableSlug: string, pointSlug: string): Promise<TimeSeriesData> {
  const response = await fetch(`/simulation/scenarios/${scenarioSlug}/${variableSlug}/timeSeries/${pointSlug}.json`)
  if (!response.ok) {
...
function makeSlugForSingleScenario(scenarioSlug: string, variableSlug: string, pointSlug: string): string {
...
async function fetchSimulationResultTimeSeries(key: string): Promise<TimeSeriesData> {
  const [scenarioASlug, variableSlug, pointSlug] = key.split(';').map(s => (s === "_" || s === "null") ? null : s);
  return fetchSimulationResultTimeSeriesForScenarioVariableAndPoint(scenarioASlug!, variableSlug!, pointSlug!);
}
...
export const useSimulationResultTimeSeriesStore = defineStore('simulationResultTimeSeries', () => {
  ...
  async function getSimulationResultTimeSeries(scenarioASlug: string, variableSlug: string, pointSlug: string): Promise<TimeSeriesData> {
