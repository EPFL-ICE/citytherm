...
export const useSelections = defineStore('selections', {
  ...
  actions: {
    ...
    toggleFromFeature(
      f: Feature<Polygon | MultiPolygon | Point, any>,
      cellIdProp = 'cell_id'
    ) {
...
function ensureSelectedLayers() {
  ...
  if (!map.getLayer(LAYER_SELECTED_CIRCLES)) {
    ...
    }, props.insertAboveLayerId); // insert before if provided
  }
  // label layer
...
onMounted(() => {
  ...
  const stopWatch = watch(() => selections.featureCollection, (fc) => {
...
.counter { margin-left: auto; opacity: 0.7; }
.sel-table { width: 100%; border-collapse: collapse; }
.sel-table th, .sel-table td { border: 1px solid #eee; padding: 6px; vertical-align: top; }
