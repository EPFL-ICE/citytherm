...
variable_categories = {
    ...
    "facade_conv_heat_transfer_coeff": {
...
def process_netcdf(scenario_name: str, input_directory: str, output_directory: str):
    ...
    var_keys = underground_level_variables + ground_level_variables + surface_level_variables + building_data_variables
...
def objects_dict_scenarios(scenario_name: str, objects):
    if scenario_name.startswith("S4_2") or scenario_name.startswith("S6_4"): # betula trees
        return objects_dict_trees(-2)
    elif scenario_name.startswith("S6_1"): # betula trees staggered
        return objects_dict_trees_staggered(-2)
    elif scenario_name.startswith("S4_3") or scenario_name.startswith("S6_3"): # acer trees
        return objects_dict_trees(-3)
    elif scenario_name.startswith("S6_5"): # acer trees
        return objects_dict_trees_wide(-3)
    elif scenario_name.startswith("S5_1"): # mist nozzles
        return objects_dict_water_bodies(-10)
    elif scenario_name.startswith("S6_2"): # mist nozzles
        return objects_dict_water_bodies(-10, stagger=True)
...
def objects_dict_trees_staggered(type: int = -2):
    return {
        "defaultObject": type,
        "objects": [ # hardcoded list provided by Jaafar, values seem to be the index of the 2x2 square, so multiplied by 2 here
            { "x": 30 * 2, "y": 63 * 2 },
...
def objects_dict_trees_wide(type: int = -2):
    return {
        "defaultObject": type,
        "objects": [ # hardcoded list provided by Jaafar, values seem to be the index of the 2x2 square, so multiplied by 2 here
            { "x": 23 * 2, "y": 91 * 2 },
...
def objects_dict_water_bodies(type: int = -10, stagger: bool = False):
    offset = 9 if stagger else 0
    return {
        "defaultObject": type,
        "objects": [
            { "x": 30 * 2, "y": 18 * 2 },
            ...
            { "x": 66 * 2, "y": 27 * 2 },
            { "x": 69 * 2, "y": 27 * 2 },
            { "x": (30 + offset) * 2, "y": 36 * 2 },
            { "x": (33 + offset) * 2, "y": 36 * 2 },
            { "x": (48 + offset) * 2, "y": 36 * 2 },
            { "x": (51 + offset) * 2, "y": 36 * 2 },
            { "x": (66 + offset) * 2, "y": 36 * 2 },
            { "x": (69 + offset) * 2, "y": 36 * 2 },
            { "x": (30 + offset) * 2, "y": 37 * 2 },
            { "x": (33 + offset) * 2, "y": 37 * 2 },
            { "x": (48 + offset) * 2, "y": 37 * 2 },
            { "x": (51 + offset) * 2, "y": 37 * 2 },
            { "x": (66 + offset) * 2, "y": 37 * 2 },
            { "x": (69 + offset) * 2, "y": 37 * 2 },
            { "x": (30 + offset) * 2, "y": 38 * 2 },
            { "x": (33 + offset) * 2, "y": 38 * 2 },
            { "x": (48 + offset) * 2, "y": 38 * 2 },
            { "x": (51 + offset) * 2, "y": 38 * 2 },
            { "x": (66 + offset) * 2, "y": 38 * 2 },
            { "x": (69 + offset) * 2, "y": 38 * 2 },
            { "x": (30 + offset) * 2, "y": 39 * 2 },
            { "x": (33 + offset) * 2, "y": 39 * 2 },
            { "x": (48 + offset) * 2, "y": 39 * 2 },
            { "x": (51 + offset) * 2, "y": 39 * 2 },
            { "x": (66 + offset) * 2, "y": 39 * 2 },
            { "x": (69 + offset) * 2, "y": 39 * 2 },
            { "x": (30 + offset) * 2, "y": 40 * 2 },
            { "x": (33 + offset) * 2, "y": 40 * 2 },
            { "x": (48 + offset) * 2, "y": 40 * 2 },
            { "x": (51 + offset) * 2, "y": 40 * 2 },
            { "x": (66 + offset) * 2, "y": 40 * 2 },
            { "x": (69 + offset) * 2, "y": 40 * 2 },
            { "x": (30 + offset) * 2, "y": 41 * 2 },
            { "x": (33 + offset) * 2, "y": 41 * 2 },
            { "x": (48 + offset) * 2, "y": 41 * 2 },
            { "x": (51 + offset) * 2, "y": 41 * 2 },
            { "x": (66 + offset) * 2, "y": 41 * 2 },
            { "x": (69 + offset) * 2, "y": 41 * 2 },
            { "x": (30 + offset) * 2, "y": 42 * 2 },
            { "x": (33 + offset) * 2, "y": 42 * 2 },
            { "x": (48 + offset) * 2, "y": 42 * 2 },
            { "x": (51 + offset) * 2, "y": 42 * 2 },
            { "x": (66 + offset) * 2, "y": 42 * 2 },
            { "x": (69 + offset) * 2, "y": 42 * 2 },
            { "x": (30 + offset) * 2, "y": 43 * 2 },
            { "x": (33 + offset) * 2, "y": 43 * 2 },
            { "x": (48 + offset) * 2, "y": 43 * 2 },
            { "x": (51 + offset) * 2, "y": 43 * 2 },
            { "x": (66 + offset) * 2, "y": 43 * 2 },
            { "x": (69 + offset) * 2, "y": 43 * 2 },
            { "x": (30 + offset) * 2, "y": 44 * 2 },
            { "x": (33 + offset) * 2, "y": 44 * 2 },
            { "x": (48 + offset) * 2, "y": 44 * 2 },
            { "x": (51 + offset) * 2, "y": 44 * 2 },
            { "x": (66 + offset) * 2, "y": 44 * 2 },
            { "x": (69 + offset) * 2, "y": 44 * 2 },
            { "x": (30 + offset) * 2, "y": 45 * 2 },
            { "x": (33 + offset) * 2, "y": 45 * 2 },
            { "x": (48 + offset) * 2, "y": 45 * 2 },
            { "x": (51 + offset) * 2, "y": 45 * 2 },
            { "x": (66 + offset) * 2, "y": 45 * 2 },
            { "x": (69 + offset) * 2, "y": 45 * 2 },
            { "x": 30 * 2, "y": 54 * 2 },
            { "x": 33 * 2, "y": 54 * 2 },
            { "x": 48 * 2, "y": 54 * 2 },
            { "x": 51 * 2, "y": 54 * 2 },
            ...
            { "x": 66 * 2, "y": 63 * 2 },
            { "x": 69 * 2, "y": 63 * 2 },
            { "x": (30 + offset) * 2, "y": 72 * 2 },
            { "x": (33 + offset) * 2, "y": 72 * 2 },
            { "x": (48 + offset) * 2, "y": 72 * 2 },
            { "x": (51 + offset) * 2, "y": 72 * 2 },
            { "x": (66 + offset) * 2, "y": 72 * 2 },
            { "x": (69 + offset) * 2, "y": 72 * 2 },
            { "x": (30 + offset) * 2, "y": 73 * 2 },
            { "x": (33 + offset) * 2, "y": 73 * 2 },
            { "x": (48 + offset) * 2, "y": 73 * 2 },
            { "x": (51 + offset) * 2, "y": 73 * 2 },
            { "x": (66 + offset) * 2, "y": 73 * 2 },
            { "x": (69 + offset) * 2, "y": 73 * 2 },
            { "x": (30 + offset) * 2, "y": 74 * 2 },
            { "x": (33 + offset) * 2, "y": 74 * 2 },
            { "x": (48 + offset) * 2, "y": 74 * 2 },
            { "x": (51 + offset) * 2, "y": 74 * 2 },
            { "x": (66 + offset) * 2, "y": 74 * 2 },
            { "x": (69 + offset) * 2, "y": 74 * 2 },
            { "x": (30 + offset) * 2, "y": 75 * 2 },
            { "x": (33 + offset) * 2, "y": 75 * 2 },
            { "x": (48 + offset) * 2, "y": 75 * 2 },
            { "x": (51 + offset) * 2, "y": 75 * 2 },
            { "x": (66 + offset) * 2, "y": 75 * 2 },
            { "x": (69 + offset) * 2, "y": 75 * 2 },
            { "x": (30 + offset) * 2, "y": 76 * 2 },
            { "x": (33 + offset) * 2, "y": 76 * 2 },
            { "x": (48 + offset) * 2, "y": 76 * 2 },
            { "x": (51 + offset) * 2, "y": 76 * 2 },
            { "x": (66 + offset) * 2, "y": 76 * 2 },
            { "x": (69 + offset) * 2, "y": 76 * 2 },
            { "x": (30 + offset) * 2, "y": 77 * 2 },
            { "x": (33 + offset) * 2, "y": 77 * 2 },
            { "x": (48 + offset) * 2, "y": 77 * 2 },
            { "x": (51 + offset) * 2, "y": 77 * 2 },
            { "x": (66 + offset) * 2, "y": 77 * 2 },
            { "x": (69 + offset) * 2, "y": 77 * 2 },
            { "x": (30 + offset) * 2, "y": 78 * 2 },
            { "x": (33 + offset) * 2, "y": 78 * 2 },
            { "x": (48 + offset) * 2, "y": 78 * 2 },
            { "x": (51 + offset) * 2, "y": 78 * 2 },
            { "x": (66 + offset) * 2, "y": 78 * 2 },
            { "x": (69 + offset) * 2, "y": 78 * 2 },
            { "x": (30 + offset) * 2, "y": 79 * 2 },
            { "x": (33 + offset) * 2, "y": 79 * 2 },
            { "x": (48 + offset) * 2, "y": 79 * 2 },
            { "x": (51 + offset) * 2, "y": 79 * 2 },
            { "x": (66 + offset) * 2, "y": 79 * 2 },
            { "x": (69 + offset) * 2, "y": 79 * 2 },
            { "x": (30 + offset) * 2, "y": 80 * 2 },
            { "x": (33 + offset) * 2, "y": 80 * 2 },
            { "x": (48 + offset) * 2, "y": 80 * 2 },
            { "x": (51 + offset) * 2, "y": 80 * 2 },
            { "x": (66 + offset) * 2, "y": 80 * 2 },
            { "x": (69 + offset) * 2, "y": 80 * 2 },
            { "x": (30 + offset) * 2, "y": 81 * 2 },
            { "x": (33 + offset) * 2, "y": 81 * 2 },
            { "x": (48 + offset) * 2, "y": 81 * 2 },
            { "x": (51 + offset) * 2, "y": 81 * 2 },
            { "x": (66 + offset) * 2, "y": 81 * 2 },
            { "x": (69 + offset) * 2, "y": 81 * 2 }
        ]
    }
...
def hardcoded_overrides(variable_name: str, attrs: dict):
    if variable_name == "UTCI" or variable_name == "PET":
        ...
        attrs["group"] = "surface_level"
    elif variable_name in building_data_variables:
        attrs["available_at"] = [human_height, 17.0, 31.0]
        attrs["group"] = "building_data"
    elif variable_name in underground_level_variables:
        attrs["available_at"] = [-0.25, -1.250]
        attrs["group"] = "underground_level"
    else:
        attrs["available_at"] = [0.2, human_height, 17.0, 31.0]
...
def save_plane_slices_for_var_at_time(scenario: str, ds, output_directory: str, variable_slug="T", time_index=0):
    ...
    slicers = get_underground_plane_slicers_for_scenario(scenario) if variable_slug in underground_level_variables else get_plane_slicers_for_scenario(scenario)
    ...
    return [
        {
            ...
            "slicer": lambda df: slice_xy_plane_at_z(df, 0.250),
        },
        {
...
def make_time_series_point(name: str, slug: str, coords: list[float], available_variables: list[str], depth_variables: list[str], corresponding_plane: str):
    return {
        "n": name,
        "s": slug,
        "c": coords,
...
def get_single_time_series_point_for_var_and_coords_dataframe(ds, variable_name: str, coords: list[float], filter_nan: bool = False):
    ...
    elif "SoilLevels" in variable.dims and len(coords) > 2:
        selection["SoilLevels"] = abs(coords[2])
        columns_to_drop.append("SoilLevels")
...
def get_depth_series_for_var_coords_and_time_dataframe(ds, variable_name: str, coords: list[float], filter_nan: bool = False):
    ...
    df = point_data.to_dataframe().reset_index().rename(columns={variable_name: "v"}).drop(columns=columns_to_drop)
    depths = sorted(df["SoilLevels"].unique())
    result = {
