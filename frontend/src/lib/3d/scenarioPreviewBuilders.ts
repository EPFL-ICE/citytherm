...
export function createSoilGeometry(sceneSize: Vector2, quadSize: number, soilMap: SoilMap) {
  ...
  for (let i = 0; i < uvs.count; i += 6) { // 6 vertices per quad (two triangles)
    const quadIndex = (i / 6);
    ...
    const color = simulationSoilTypeCodeToColor(correspondingAnomaly ? correspondingAnomaly.t : soilMap.defaultSoilType);
...
function getTransformationMatrixForBuilding(sceneSize: Vector2, building: BuildingPart, defaultHeight: number, dummyObject?: THREE.Object3D) {
...
export function createBuildingInstancedMesh(buildingMap: BuildingMap, sceneSize: Vector2) {
  ...
  buildingMap.buildingsParts.forEach((building, i) => {
    ...
    const matrix = getTransformationMatrixForBuilding(sceneSize, building, buildingMap.defaultHeight, dummy);
    mesh.setMatrixAt(i, matrix);

    // Set colors based on height
    const colorTop = new THREE.Color().setHex(building.tc ? stringHexCodeToHexNumber(building.tc) : defaultTopColor);
    const colorSide = new THREE.Color().setHex(building.sc ? stringHexCodeToHexNumber(building.sc) : defaultSideColor);
